...
let words = [
    ...
    "paddy", "pagan", "paint", "paler", "palsy", "panel", "panic", "pansy", "papal", "paper", "parer", "parka", "parry", "parse", "party", "pasta", "paste", "pasty", "patch", "patio", "patsy", "patty", "pause", "payee", "payer", "peace", "peach", "pearl", "pecan", "pedal", "penal", "pence", "penne", "penny", "perch", "peril", "perky", "pesky", "pesto", "petal", "petty", "phase", "pique", "pitch", "pithy", "pivot", "pixel", "pixie", "pizza", "place", "plaid", "plain", "plait", "plane", "plank", "plant", "plate", "plaza", "plead", "pleat", "plied", "plier", "pluck", "plumb", "plume", "plump", "plunk", "plush", "poesy", "point", "poise", "poker", "polar", "polka", "polyp", "pooch", "poppy", "porch", "poser", "posit", "posse", "pouch", "pound", "pouty", "power", "prank", "prawn", "preen", "press", "price", "prick", "pride", "pried", "prime", "primo", "print", "prior", "prism", "privy", "prize", "probe", "prone", "prong", "proof", "prose", "proud", "prove", "prowl", "proxy", "prude", "prune", "psalm", "pubic", "pudgy", "puffy", "pulpy", "pulse", "punch", "pupil", "puppy", "puree", "purer", "purge", "purse", "pushy", "putty",
    "pygmy",
    ...
    "syrup",
    "tabby", "table", "taboo", "tacit", "tacky", "taffy", "taint", "taken", "taker", "tally", "talon", "tamer", "tango", "tangy",
    "taper", "tapir", "tardy", "tarot", "taste", "tasty", "tatty", "taunt", "tawny", "teach", "teary", "tease", "teddy", "teeth",
    "tempo", "tenet", "tenor", "tense", "tenth", "tepee", "tepid", "terra", "terse", "testy", "thank", "theft", "their", "theme",
    "there", "these", "theta", "thick", "thief", "thigh", "thing", "think", "third", "thong", "thorn", "those", "three", "threw",
    "throb", "throw", "thrum", "thumb", "thump", "thyme", "tiara", "tibia", "tidal", "tiger", "tight", "tilde", "timer", "timid",
    "tipsy", "titan", "tithe", "title", "toast", "today", "toddy", "token", "tonal", "tonga", "tonic", "tooth", "topaz", "topic",
    "torch", "torso", "torus", "total", "totem", "touch", "tough", "towel", "tower", "toxic", "toxin", "trace", "track", "tract",
    "trade", "trail", "train", "trait", "tramp", "trash", "trawl", "tread", "treat", "trend", "triad", "trial", "tribe", "trice",
    "trick", "tried", "tripe", "trite", "troll", "troop", "trope", "trout", "trove", "truce", "truck", "truer", "truly", "trump",
    "trunk", "truss", "trust", "truth", "tryst", "tubal", "tuber", "tulip", "tulle", "tumor", "tunic", "turbo", "tutor", "twang",
...
let word = words[randomNumber].toUpperCase();
...
function playGame() {
    document.getElementById('title').style.margin = '5rem 0';
    document.getElementById('playButton').style.display = 'none';
    document.getElementById('wordleGrid').style.display = 'grid';
}
...
function initialize() {
    for (let r = 0; r < height; r++) {
        for (let c = 0; c < width; c++) {
            ...
            tile.innerText = "";
            document.getElementById("wordleGrid").appendChild(tile);
        }
    }
    ...
    document.addEventListener("keyup", (e) => {
        ...
        if ("KeyA" <= e.code && e.code <= "KeyZ") {
            if (col < width) {
                ...
                if (currentTile.innerText == "") {
                    ...
                    currentTile.classList.add("wiggle");
                    col++;
                }
            }
            ...
            if (0 < col && col <= width) {
                ...
                currentTile.classList.remove("wiggle");
            }
        ...
        if (!gameOver && row == height) {
            gameOver == true;
            document.getElementById("answer").innerText = "Correct Answer: " + word;
        }
    })
}

function flipTile(row, col, type, delay = 0) {
    let tile = document.getElementById(row + "-" + col);

    let animationClass = "";
    let finalColor = "";
    if (type === "absent") { animationClass = "flip"; finalColor = "#787c7e"; }
    else if (type === "present") { animationClass = "flip-p"; finalColor = "#c9b458"; }
    else if (type === "correct") { animationClass = "flip-c"; finalColor = "#6AAA64"; }
    ...
    tile.addEventListener("animationend", () => {
        ...
        tile.style.background = finalColor;
...
function update() {
    ...
    let guess = document.getElementById(row + "-" + 0).textContent + document.getElementById(row + "-" + 1).textContent + document.getElementById(row + "-" + 2).textContent + document.getElementById(row + "-" + 3).textContent + document.getElementById(row + "-" + 4).textContent;

    let goodGuess = false;

    for (let i = 0; i < words.length; i++) {
        if (guess === words[i].toUpperCase()) {
            goodGuess = true;
        }
    }

    if (!goodGuess) {
        alert("The guess you made is not on the word list, please try another word.");
        ...
        for (let c = 0; c < width; c++) {
            ...
            if (word[c] == letter) {
                flipTile(row, c, "correct", 100 + c * 250);
                correct++;
            } else if (word.includes(letter)) {
                flipTile(row, c, "present", 100 + c * 250);
            } else {
                flipTile(row, c, "absent", 100 + c * 250);
            }
            ...
            if (correct == width) {
                ...
                setTimeout(() => {
                    document.getElementById('answer').textContent = "Congratulations, you guessed the word!";

                    var end = Date.now() + (1 * 1000);
                    var colors = ['#2ab91dff', '#015005ff', '#ffffffff',];
